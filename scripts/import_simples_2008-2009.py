...
def main(argv):
	...
	for cells in lines:
		...
		if rowcount < 4:
			...
			schoolname = cells[1].replace('"','')
			bedscode = cells[0].replace('"','')
			#gradeorg = cells[4].replace('"','')
			#resoucecat = cells[5].replace('"','')
			schooltype = cells[3].replace('"','')
			cell4 = cells[4].replace('"','')
			if cell4 != "":
				enrollment = long(float(cell4))
			else:
				enrollment = 0
			...
			if helper.check_district_exists(district) == False:
				...
				print "District '{0}' is unknown, possible database coruption.".format(district)
